...
class ImprovedLocalSearch:
    ...
    def solve(self) -> List[int]:
        ...
        best_size = len(best)
        last_best_step = 0
        strong_start = max(100, min(500, self.max_steps // 6))
        plateau_limit = max(800, self.max_steps // 2)

        for step in range(self.max_steps):
            improved = False
            plateau_steps = step - last_best_step
            ...
            if (
                not improved
                and plateau_steps >= strong_start
                and step % 8 == 0
            ...
            if step - last_best_step >= plateau_limit:
                LOGGER.info(
                    f"  Early stop at step {step + 1}: no improvement for "
                    f"{plateau_limit} steps"
                )
                break
    ...
    def _sample_structural_add_candidates(
        ...
        limit = sample_size or self.candidate_sample_size
        pool = set()
        for pivot in pivots:
    ...
    def _try_add_then_remove_many(self, tracker: CoverageTracker, step: int) -> bool:
        ...
        pivots = heapq.nsmallest(
            min(4, len(tracker.in_solution)),
            tracker.in_solution,
            key=tracker.exclusive_count,
        )
        structural_sample_size = max(24, self.candidate_sample_size // 2)
        add_pool = self._sample_structural_add_candidates(
            tracker, pivots, sample_size=structural_sample_size
        )
        if not add_pool:
            return False
        ...
        snapshot = list(tracker.in_solution)
        trials = min(2, max(1, len(add_pool)))
        ...
        for trial in range(trials):
            ...
            if len(add_pool) <= add_count:
                ...
                top_window = add_pool[: min(len(add_pool), structural_sample_size)]
    ...
    def _try_destroy_repair(self, tracker: CoverageTracker, step: int) -> bool:
        ...
        snapshot = list(tracker.in_solution)
        size_before = tracker.solution_size
        max_destroy = min(5, max(2, len(snapshot) // 25), len(snapshot) - 1)
        destroy_count = self.rng.randint(2, max_destroy)
        destroy_pool = heapq.nsmallest(
            min(max_destroy * 2, len(snapshot)),
            snapshot,
...
class ImprovedSA:
    ...
    def solve(self) -> List[int]:
        ...
        for iteration in range(self.max_iter):
            ...
            allow_strong_move = no_improve >= 100
            if not allow_strong_move:
                if draw < 0.35:
                ...
                move = self._move_add_then_remove_many(tracker, outside)
            else:
                move = self._move_destroy_repair(tracker, outside)
