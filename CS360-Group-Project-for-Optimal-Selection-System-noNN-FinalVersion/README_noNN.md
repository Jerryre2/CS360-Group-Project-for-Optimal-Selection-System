...
这个 noNN 副本现在同步了主线中的非神经网络功能：

- `validate-result` / `validate-file` / `validate-results`：验证保存的结果是否满足覆盖约束。
- `add-then-remove-many`：先加入 1 到 3 个结构相关候选，再删除多个冗余组。
- `destroy-repair`：删除一批低贡献组，再用相关候选 greedy repair。
- `--exact-backend auto|scipy|gurobi|scip`、`--force-exact`、`prove-bound`：用于更强的 exact verification。
...
python -m optimal_samples_system prove-bound --m 45 --n 15 --k 6 --j 5 --s 4 \
  ...
  --target-size 48 --exact-backend auto --exact-time-limit 300
```

说明：该副本不包含实际 NN 模型或在线训练逻辑，`--disable-neural-guidance` 只是为了兼容旧命令，行为上没有影响。
